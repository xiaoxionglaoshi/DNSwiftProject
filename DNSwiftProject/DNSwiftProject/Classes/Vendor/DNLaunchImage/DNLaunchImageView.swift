...
class DNLaunchImageView: UIView {
    
    fileprivate var imageViewDefault: UIImageView!
    fileprivate var bgImageView: DNLaunchImageAction!
    ...
    var adsViewCompletion: (DNLaunchImageView) -> Void = {
        (launchImageView: DNLaunchImageView) in
    }
    ...
    init(imageUrl: String, clickImageAction: @escaping () -> ()) {
        ...
        imageViewDefault.image = DNLaunchImage.getSystemLaunchImage()
        addSubview(imageViewDefault)
        bgImageView = DNLaunchImageAction.init(frame: UIScreen.main.bounds)
        ...
        if cacheResult.cached {
            ...
            bgImageView.kf.setImage(with: ImageResource(downloadURL: URL(string: imageUrl)!), placeholder: DNLaunchImage.getSystemLaunchImage(), options: [.transition(ImageTransition.fade(1))], progressBlock: nil, completionHandler: nil)
            isImageDownLoad = true
            
        }else{
            timeButton.isHidden = true
            bgImageView.image = DNLaunchImage.getSystemLaunchImage()
    ...
    public class func startAdsWith(imageUrl: String, clickImageAction: @escaping () -> ()) -> Any {
        return DNLaunchImageView.init(imageUrl: imageUrl, clickImageAction: clickImageAction)
    }
    
    func startAnimationTime(time: Int, completionBlock: @escaping (DNLaunchImageView) -> Void) {
        timeNum = time
