...
private let sn_alpha: CGFloat = 0.8
...
extension UIResponder {
    ...
    func noticeSuccess(_ text: String, autoClear: Bool = false, autoClearTime: Int = 2) {
        SwiftNotice.showNoticeWithText(NoticeType.success, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func noticeError(_ text: String, autoClear: Bool = false, autoClearTime: Int = 2) {
        SwiftNotice.showNoticeWithText(NoticeType.error, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func noticeInfo(_ text: String, autoClear: Bool = false, autoClearTime: Int = 2) {
    ...
    func infoNotice(_ text: String, autoClear: Bool = true) {
        SwiftNotice.showNoticeWithText(NoticeType.info, text: text, autoClear: autoClear, autoClearTime: 2)
    }
    
    func notice(_ text: String, type: NoticeType, autoClear: Bool, autoClearTime: Int = 2) {
        SwiftNotice.showNoticeWithText(type, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func pleaseWait(messageText: String) {
        SwiftNotice.wait(messageText: messageText)
    }
...
class SwiftNotice: NSObject {
    ...
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0, messageText: String = "加载中...") {
        let frame = CGRect(x: 0, y: 0, width: 90, height: 90)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        mainView.layer.cornerRadius = 12
        mainView.backgroundColor = UIColor(red:0, green:0, blue:0, alpha: sn_alpha)
        ...
        if imageNames.count > 0 {
            ...
            let textLabel = UILabel(frame: CGRect(x: 8, y: frame.height - 30, width: frame.width - 16, height: 15))
