...
import UIKit
import pop
class DNComposeTypeView: UIView {
    ...
    /// 按钮数据数组
    fileprivate let buttonsInfo = [["imageName": "tabbar_compose_idea", "title": "文字","clsName": "DNComposeViewController"],
    ...
    // MARK: - 实例化方法
    class func composeTypeView() ->DNComposeTypeView {
    
        let nib = UINib.init(nibName: "DNComposeTypeView", bundle: nil)
        let view = nib.instantiate(withOwner: nil, options: nil)[0] as! DNComposeTypeView
    ...
    //按钮点击事件
    @objc fileprivate func btnAction(selectedbutton: DNComposeTypeButton) {
...
//MARK: - 动画方法扩展
fileprivate extension DNComposeTypeView {
    ...
    func addBtns(view:UIView, index:Int) {
        ...
        for i in index..<(index + count) {
           ...
           guard let imageName = dic["imageName"],
            ...
            //创建按钮
            let btn = DNComposeTypeButton.composeTypeButton(imageName:imageName , title: title)
